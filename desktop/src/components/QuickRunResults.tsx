      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <ActivityIcon className={cn("size-4", isRunning ? "animate-pulse text-sky-600 dark:text-sky-400" : "text-muted-foreground")} />
            <h3 className="text-sm font-semibold text-foreground">执行状态</h3>
          </div>
          <Badge variant={isRunning ? "default" : "outline"} className={cn("font-mono text-[10px]", isRunning && "bg-sky-500")}>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
        <div className="flex items-center justify-between border-b border-border bg-muted/40 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">最近执行记录</span>
          </div>
        ...
        <ScrollArea className="flex-1">
          <div className="divide-y divide-border">
            ...
            {displayItems.length === 0 ? (
              <div className="p-8 text-center text-xs italic text-muted-foreground">暂无明细记录</div>
            ) : (
                  ...
                  >
                    <div className="flex min-w-0 items-center gap-2">
                      <div className={cn(
                        ...
                        {isSuccess ? <CheckCircle2Icon className="size-3" /> : <AlertCircleIcon className="size-3" />}
                      </div>
                      <div className="min-w-0">
                        <span className="block truncate text-xs font-mono font-medium text-foreground">
                          {item.shop_code}
                        </span>
                        <span className="block text-[10px] font-mono text-muted-foreground">#{item.index}</span>
                      </div>
                    </div>
                    <div className="min-w-0">
