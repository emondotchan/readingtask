...
const SHOP_TYPE_AVENE = 0;
const SHOP_TYPE_KLORANE = 1;
const SHOP_TYPE_AVENE_KLORANE = 2;
...
function buildMonthlyTaskId(courseId: string, managerId: string, taskType: TaskType) {
...
function parseMultilineValues(raw: string) {
  ...
  for (const [index, line] of raw.split(/\r?\n/).entries()) {
    const value = (index === 0 ? line.trimStart().replace(/^\uFEFF/, "") : line).trim();
...
function taskTypeMatchesShop(taskType: TaskType, shopType: number) {
  if (taskType === "Avene") {
    return shopType === SHOP_TYPE_AVENE || shopType === SHOP_TYPE_AVENE_KLORANE;
  }

  return shopType === SHOP_TYPE_KLORANE || shopType === SHOP_TYPE_AVENE_KLORANE;
}

function getTaskTypeSortOrder(taskType: TaskType) {
...
export default function MonthlyPlanManager({
  ...
  const loadData = useCallback(async () => {
    ...
    for (const task of ts) {
      try {
        ...
        const pendingTask = dailyTasks.find((item) => !isCompletedDay(item) && !item.is_locked);
        const activeTask = pendingTask ?? dailyTasks.find((item) => item.date === today) ?? null;
        progressEntries.push([task.id, activeTask] as const);
  ...
  const groupedTasks = useMemo(() => {
    const groups = new Map<string, MonthlyTask[]>();

    for (const task of filteredTasks) {
      const group = groups.get(task.fc_name);
      if (group) {
        group.push(task);
      } else {
        groups.set(task.fc_name, [task]);
      }
    }

    return Array.from(groups, ([fcName, groupTasks]) => ({
      fcName,
      tasks: [...groupTasks].sort((a, b) => {
        const typeOrder = getTaskTypeSortOrder(a.task_type) - getTaskTypeSortOrder(b.task_type);

        return typeOrder || a.id.localeCompare(b.id);
      }),
    }));
  }, [filteredTasks]);
  ...
  useEffect(() => {
    ...
    const previewInputs = (["Avene", "Klorane"] as const)
      .map((taskType) => {
        ...
        const readingUrl = taskType === "Avene" ? aveneReadingUrl : kloraneReadingUrl;
        ...
        const task: MonthlyTask = {
          id: buildMonthlyTaskId(parsedReadingLink.courseId, parsedReadingLink.managerId, taskType),
          fc_name: fc.name,
      ...
      .filter((item): item is readonly [TaskType, MonthlyTask] => item !== null);
    ...
    <div className="flex w-full flex-col gap-6">
      {selectedTask ? (
        <TaskStatusDialog
          ...
          executeDaily={executeDaily}
          pauseDaily={pauseDaily}
        />
      ) : (
          ...
          <CardHeader className="border-b border-border pb-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg font-semibold">月度计划</CardTitle>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Select value={filterMonth} onValueChange={setFilterMonth}>
                  ...
                  <SelectContent>
                    {monthOptions.map((opt: { label: string; value: string }) => (
                ...
                <Select
                  ...
                  onValueChange={(value) => setTaskTypeFilter(value as "all" | "Avene" | "Klorane")}
                >
                  <SelectTrigger className="h-8 w-[140px] border-border bg-background/70 text-xs">
              ...
              <Table className="min-w-[1080px] table-fixed">
                <TableHeader>
                  <TableRow className="text-muted-foreground hover:bg-transparent">
                    <TableHead className="sticky top-0 z-10 w-[170px] bg-card px-5 text-center text-xs font-semibold uppercase shadow-[0_1px_0_0_var(--color-border)]">
                      FC姓名
                    </TableHead>
                    <TableHead className="sticky top-0 z-10 w-[100px] bg-card text-center text-xs font-semibold uppercase shadow-[0_1px_0_0_var(--color-border)]">
                      类型
                    </TableHead>
                ...
                <TableBody>
                  {filteredTasks.length === 0 ? (
                    ...
                    groupedTasks.flatMap((group) =>
                      group.tasks.map((task: MonthlyTask, taskIndex) => {
                        const prog = progressMap[task.id];
                        const taskRun = getTaskRun(task.id);
                        const dailyRunStatus = getDailyTaskRunStatus(prog);
                        const displayRunState = taskRun?.runState ?? dailyRunStatus;
                        const isCompletedToday = Boolean(prog && isCompletedDay(prog));
                                ...
                                {!isCompletedToday && !isCompletedMonth && (
                                  <Button
                                    ...
                                    onClick={() => {
                                      ...
                                      void executeDaily(task.id, prog?.date ?? today);
                                    }}
                                  >
                                    {isRunning ? (
                                      <SquareIcon />
      ...
      >
        <DialogContent className="sm:max-w-3xl max-h-[88vh] overflow-hidden flex flex-col">
          <DialogHeader className="border-b pb-4">
            ...
            <DialogDescription>
              选择 FC 并分别输入 Avene 与 Klorane 阅读链接，系统会一次创建两类月度任务。
            </DialogDescription>
          </DialogHeader>
            ...
            <form className="flex flex-col gap-5" onSubmit={handleCreate}>
              ...
              <FieldGroup>
                <Field>
                  ...
                  <Select value={fcName} onValueChange={setFcName} disabled={fcs.length === 0}>
                    <SelectTrigger id="plan-fc" className="w-full data-[size=default]:h-9">
                      <SelectValue placeholder={fcs.length === 0 ? "暂无可选 FC" : "选择 FC"} />
                    </SelectTrigger>
                    <SelectContent>
                      {fcs.length === 0 ? (
                ...
                {(["Avene", "Klorane"] as const).map((taskType) => {
                  ...
                  return (
                    <Field key={taskType}>
                      <FieldLabel htmlFor={fieldId}>{taskType} 阅读链接</FieldLabel>
                      <Input
                        id={fieldId}
                        value={readingUrl}
                      ...
                      {readingUrl.trim() && !parsedReadingLink && (
                        <p className="text-xs text-destructive">
                          链接需包含 CourseID 与 UID，或在 redirect_uri 中包含 CourseID 与 UID。
                        </p>
                      )}
                      {parsedReadingLink && !selectedCourse && (
                        <p className="text-xs text-destructive">
                          当前月份课程配置中未找到 CourseID=
                          {parsedReadingLink.courseId} 且类型为 {taskType} 的课程。
                        </p>
                      )}
                      {parsedReadingLink && selectedCourse && (
                ...
                <Field>
                  <FieldLabel htmlFor="plan-shopcodes">自定义 Shopcodes</FieldLabel>
                  <Textarea
                    id="plan-shopcodes"
                    rows={6}
                    className="max-h-48 overflow-y-auto"
                    placeholder={"留空则按系统规则随机生成\n如需指定，请一行一个 shopcode"}
                    value={customShopcodesText}
                    onChange={(event) => setCustomShopcodesText(event.target.value)}
                  />
                </Field>

                <Field>
                  <FieldLabel htmlFor="plan-excluded-openids">排除 OpenIDs</FieldLabel>
                  <Textarea
                    id="plan-excluded-openids"
                    rows={6}
                    className="max-h-48 overflow-y-auto"
                    placeholder={"可选，一行一个 OpenID\n执行时不会使用这些 OpenID"}
                    value={excludedOpenIdsText}
                    onChange={(event) => setExcludedOpenIdsText(event.target.value)}
                  />
                </Field>
                ...
                {Object.keys(taskPreviews).length > 0 && (
                  <div className="flex flex-col gap-3">
                    {(customShopcodes.length > 0 || excludedOpenIds.length > 0) && (
                      <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                        ...
                        {excludedOpenIds.length > 0 && (
                          <Badge variant="outline">排除 openids {excludedOpenIds.length} 条</Badge>
                        )}
                      </div>
                    )}
                    {(["Avene", "Klorane"] as const).map((taskType) => {
